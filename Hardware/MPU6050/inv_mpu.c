...
int mpu_init(void)
{
    ...
    else
    {
        ...
        {
            log_e(
                "Product ID read as 0 indicates device is either incompatible "
                "or an MPU3050.\r\n");
            return -1;
        }
        else if (rev == 4)
...
static int gyro_self_test(long *bias_regular, long *bias_st)
{
    ...
    for (jj = 0; jj < 3; jj++)
    {
        ...
        if (tmp[jj])
        {
            ...
            (st_shift_cust < test.min_dps) || (st_shift_cust > test.max_dps))
            result |= 1 << jj;
    }
    return result;
}
