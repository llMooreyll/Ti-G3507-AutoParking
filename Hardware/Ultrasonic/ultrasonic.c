...
#define ULTRASONIC_TIMEOUT_CYCLES  (8000000U)   /* ~100ms at 80MHz for ECHO rising-edge wait */
...
uint16_t Read_Ultrasonic(void)
{
    ...
    DL_Timer_clearInterruptStatus(TIMER_ULTRASONIC_INST,
                                  DL_TIMER_INTERRUPT_LOAD_EVENT);

    /* ---------- 2. Send 10 µs trigger pulse ---------- */
    DL_GPIO_setPins(ULTRASONIC_PORT, ULTRASONIC_TRIG_PIN);
    DL_Common_delayCycles(CPUCLK_FREQ / 100000);   /* 80M / 100k = 800 cycles ≈ 10 µs */
    ...
    while (DL_GPIO_readPins(ULTRASONIC_PORT, ULTRASONIC_ECHO_PIN))
    {
        if (DL_Timer_getRawInterruptStatus(TIMER_ULTRASONIC_INST,
                                           DL_TIMER_INTERRUPT_LOAD_EVENT))
        {
    ...
    DL_Timer_stopCounter(TIMER_ULTRASONIC_INST);
    distVal = (uint16_t)((float)DL_Timer_getTimerCount(TIMER_ULTRASONIC_INST) * 0.17f);
